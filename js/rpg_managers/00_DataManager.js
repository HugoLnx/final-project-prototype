...
DataManager.loadDataFile = function(name, src, storeInBrowser) {
    ...
    xhr.onload = function() {
        if (xhr.status < 400) {
            if(storeInBrowser && localStorage[name] === undefined) localStorage[name] = xhr.responseText;
...
DataManager.loadMapData = function(mapId) {
    if (mapId > 0) {
        ...
        if(localStorage[name] === undefined) {
          this.loadDataFile(name, filename, true);
        } else {
          window[name] = JSON.parse(localStorage[name]);
          DataManager.onLoad(window[name]);
        }
    } else {
        this.makeEmptyMap();
